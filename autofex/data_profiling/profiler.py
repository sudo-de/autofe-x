...
class DataProfiler:
    ...
    def _get_overview_stats(
        ...
        return {
            ...
            'target_type': 'classification' if y is not None and self._is_classification_target(y) else 'regression'
        }
    ...
    def _analyze_missing_values(self, X: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'missing_patterns_correlation': missing_corr.to_dict() if len(missing_corr) > 0 else {}
        }
        ...
        for col in X.columns:
            ...
            if series.dtype == 'object':
                ...
                if series.dtype == 'float64':
                    ...
                    if len(non_null_series) > 0 and (non_null_series == non_null_series.astype(int)).all():
    ...
    def _analyze_distributions(self, X: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in X.columns:
            ...
            if series.dtype in ['int64', 'float64']:
                ...
                # Normality test
                try:
                    _, p_value = stats.normaltest(series.sample(min(self.sample_size, len(series))))
    ...
    def _detect_outliers(self, X: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in numeric_cols:
            ...
            outlier_report[col] = {
                ...
                'outlier_indices': outliers.index.tolist() if len(outliers) < 100 else [],  # Limit output
    ...
    def _analyze_target_relationships(self, X: pd.DataFrame, y: Union[pd.Series, np.ndarray]) -> Dict[str, Any]:
            ...
            if series.dtype == 'object':
                ...
                if numeric_series.notna().sum() > len(series) * 0.8:
                    issues.append({
                        ...
                        'conversion_rate': numeric_series.notna().sum() / len(series)
                    })
