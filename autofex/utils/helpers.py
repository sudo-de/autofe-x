...
def safe_divide(a: Union[pd.Series, np.ndarray, float],
                ...
                default: float = 0.0) -> Union[pd.Series, np.ndarray, float]:
    """
...
def get_feature_stats(series: pd.Series) -> Dict[str, Any]:
    ...
    if series.dtype in ['int64', 'float64']:
        ...
        stats.update({
            ...
            'missing_count': series.isnull().sum(),
            'missing_percentage': (series.isnull().sum() / len(series)) * 100,
            'entropy': calculate_entropy(value_counts) if len(value_counts) > 1 else 0
        })
...
def create_feature_report(features: List[str],
                         importance_scores: Optional[pd.Series] = None,
                         correlations: Optional[pd.Series] = None) -> pd.DataFrame:
    """
    ...
    for feature in features:
        feature_info = {
            'feature': feature,
            'importance_score': importance_scores[feature] if importance_scores is not None and feature in importance_scores.index else None,
            'correlation': correlations[feature] if correlations is not None and feature in correlations.index else None
        }
        report_data.append(feature_info)

    return pd.DataFrame(report_data).sort_values('importance_score', ascending=False, na_position='last')


def split_data_stratified(X: pd.DataFrame,
                         y: pd.Series,
                         test_size: float = 0.2,
