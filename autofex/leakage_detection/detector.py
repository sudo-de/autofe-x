...
class LeakageDetector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.perfect_prediction_threshold = self.config.get('perfect_prediction_threshold', 0.99)
    ...
    def detect(
        ...
        if X_test is not None:
            report['train_test_contamination'] = self._detect_train_test_contamination(X, X_test)

        if time_column and time_column in X.columns:
            report['temporal_leakage'] = self._detect_temporal_leakage(X, y, time_column)
    ...
    def _detect_target_leakage(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
            ...
            try:
                ...
                if feature.dtype in ['int64', 'float64']:
                    ...
                    if is_classification:
                        ...
                        if y.nunique() == 2:
                            corr, p_value = stats.pointbiserialr(feature.fillna(feature.mean()), y)
                        else:
                            # For multiclass, use ANOVA
                            groups = [feature[y == cls].dropna() for cls in y.unique()]
                            if len(groups) > 1 and all(len(g) > 0 for g in groups):
                                f_stat, p_value = stats.f_oneway(*groups)
                                corr = np.sqrt(f_stat / (f_stat + len(y) - len(groups))) if f_stat > 0 else 0
                            else:
                                continue
                    else:
                        # Regression: Pearson correlation
                        corr, p_value = stats.pearsonr(feature.fillna(feature.mean()), y)
                else:
        ...
        return {
            'leakage_candidates': sorted(leakage_candidates, key=lambda x: x['abs_correlation'], reverse=True),
            'threshold': self.leakage_threshold,
            'total_candidates': len(leakage_candidates)
        }

    def _detect_statistical_anomalies(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        """
        ...
        if is_classification and y.nunique() <= 10:  # Only for reasonable number of classes
                ...
                try:
                    ...
                    if len(class_stats) >= 2:
                        ...
                        if mean_range > 0:
                            ...
                            pooled_std = np.sqrt(sum(stat['std']**2 * stat['n'] for stat in class_stats) /
                                               sum(stat['n'] for stat in class_stats))
                            effect_size = mean_range / pooled_std if pooled_std > 0 else 0
                            ...
                            if effect_size > 3.0:  # Very large effect size
                                anomalies.append({
                                    ...
                                    'class_means': {stat['class']: stat['mean'] for stat in class_stats}
                                })
    ...
    def _detect_feature_perfection(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if is_classification:
                    ...
                    if feature.dtype in ['int64', 'float64']:
                        ...
                        unique_combinations = X[[col]].assign(target=y).drop_duplicates()
        ...
        for i in range(len(cols)):
            for j in range(i+1, len(cols)):
                try:
                    ...
                    if X[col1].dtype in ['int64', 'float64'] and X[col2].dtype in ['int64', 'float64']:
    ...
    def _detect_train_test_contamination(self, X_train: pd.DataFrame, X_test: pd.DataFrame) -> Dict[str, Any]:
        """
            ...
            if X_train.index.name == X_test.index.name:
                ...
                if len(overlapping_indices) > 0:
                    contamination_indicators.append({
                        ...
                        'overlap_percentage': len(overlapping_indices) / len(X_test) * 100
                    })
        ...
        for col in X_train.select_dtypes(include=[np.number]).columns:
            if col in X_test.columns:
                try:
                    ...
                    stat_diff = abs(train_stats['mean'] - test_stats['mean']) / train_stats['std']
    ...
    def _detect_temporal_leakage(self, X: pd.DataFrame, y: pd.Series, time_column: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for col in X.select_dtypes(include=[np.number]).columns:
                if col != time_column:
                    ...
                    future_corr = series.corr(target.shift(-1))  # Feature at t vs target at t+1
    ...
    def _assess_overall_leakage_risk(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Target leakage risk
        target_candidates = len(report.get('target_leakage', {}).get('leakage_candidates', []))
        if target_candidates > 0:
            risk_score += min(target_candidates * 2, 10)
            risk_factors.append(f"{target_candidates} potential target leakage features")

        # Perfect features
        perfect_count = len(report.get('feature_perfection', {}).get('perfect_features', []))
        if perfect_count > 0:
            risk_score += perfect_count * 5
            risk_factors.append(f"{perfect_count} features with perfect target prediction")

        # Statistical anomalies
        anomaly_count = report.get('statistical_anomalies', {}).get('total_anomalies', 0)
        if anomaly_count > 0:
