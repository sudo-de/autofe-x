...
class FeatureBenchmarker:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.scoring_metrics = self.config.get('scoring_metrics', ['accuracy', 'f1', 'roc_auc'])
    ...
    def benchmark_features(
        ...
        for feature_set in feature_sets:
            ...
            for col in X_subset.columns:
                if X_subset[col].dtype in ['int64', 'float64']:
                    ...
                    X_subset[col] = X_subset[col].fillna(X_subset[col].mode().iloc[0] if not X_subset[col].mode().empty else 'Unknown')

            set_results = self._benchmark_single_set(X_subset, y, set_name, is_classification)

            if X_test is not None:
                X_test_subset = X_test[features] if isinstance(features, list) else X_test
                # Basic imputation handling mixed data types
                for col in X_test_subset.columns:
                    if X_test_subset[col].dtype in ['int64', 'float64']:
                        X_test_subset[col] = X_test_subset[col].fillna(X_test_subset[col].mean())
                    else:
                        X_test_subset[col] = X_test_subset[col].fillna(X_test_subset[col].mode().iloc[0] if not X_test_subset[col].mode().empty else 'Unknown')
                set_results['test_performance'] = self._evaluate_on_test(
                    X_subset, y, X_test_subset, is_classification
                )
        ...
        # Ablation study
        results['ablation_study'] = self._perform_ablation_study(X, y, is_classification)
    ...
    def _benchmark_single_set(
        ...
        for model_name in self.models:
            try:
                model = self.model_configs[model_name]['classifier' if is_classification else 'regressor']
        ...
        for model_name in self.models:
            try:
                ...
                if is_classification:
                    ...
                    metrics = {
                        'accuracy': accuracy_score(y_train.iloc[:len(y_test)] if hasattr(y_train, 'iloc') else y_train[:len(y_test)], y_pred),
                        'precision': precision_score(y_train.iloc[:len(y_test)] if hasattr(y_train, 'iloc') else y_train[:len(y_test)], y_pred, average='weighted'),
                        'recall': recall_score(y_train.iloc[:len(y_test)] if hasattr(y_train, 'iloc') else y_train[:len(y_test)], y_pred, average='weighted'),
                        'f1': f1_score(y_train.iloc[:len(y_test)] if hasattr(y_train, 'iloc') else y_train[:len(y_test)], y_pred, average='weighted')
                    }
    ...
    def get_feature_importance(
        ...
        for col in X_filled.columns:
            if X_filled[col].dtype in ['int64', 'float64']:
                ...
                X_filled[col] = X_filled[col].fillna(X_filled[col].mode().iloc[0] if not X_filled[col].mode().empty else 'Unknown')
        ...
        for i, feature in enumerate(sorted_features):
            ...
            try:
                ...
                ablation_results.append({
                    ...
                    'relative_drop': performance_drop / baseline_score if baseline_score != 0 else 0
                })
            except Exception as e:
                ablation_results.append({
                    'removed_feature': feature,
        ...
        for model_name in self.models:
            ...
            if model_results:
                ...
                model_comparison[model_name] = {
                    ...
                    'avg_performance': np.mean([r['performance'] for r in model_results])
                }
    ...
    def _identify_best_configurations(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if all_results:
            ...
            for model_name in self.models:
                ...
                if model_results:
                    best_configs[f'best_for_{model_name}'] = max(model_results, key=lambda x: x['performance'])

        return best_configs

    def _get_baseline_score(self, X: pd.DataFrame, y: pd.Series, is_classification: bool) -> float:
        """Get baseline performance score."""
        return self._evaluate_model(X, y, is_classification)

    def _evaluate_model(self, X: pd.DataFrame, y: pd.Series, is_classification: bool) -> float:
        """Evaluate a single model on given data."""
        model = self.model_configs['rf']['classifier' if is_classification else 'regressor']
        X_filled = X.fillna(X.mean())

        cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=self.random_state) if is_classification else KFold(n_splits=3, shuffle=True, random_state=self.random_state)
        scores = cross_val_score(model, X_filled, y, cv=cv, scoring=self._get_sklearn_scorer(is_classification))
